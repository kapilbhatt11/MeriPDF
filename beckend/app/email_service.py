...
def send_verification_email(to_email: str, verify_url: str, display_name: str | None) -> bool:
    ...
    body = (
        f"Hi {greeting},\n\n"
        f"Please confirm your DocIntel account by opening this link:\n\n"
        f"{verify_url}\n\n"
    ...
    if not config.SMTP_HOST:
        logger.warning(
            "[email] SMTP not configured (DOCINTEL_SMTP_HOST empty). Verification link for %s:\n%s",
            to_email,
            verify_url,
        )
        return False
    ...
    try:
        msg = EmailMessage()
        msg["Subject"] = "Confirm your DocIntel email"
