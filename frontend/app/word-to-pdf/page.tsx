...
export default function WordToPdf() {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      let filename = `Converted_${file.name.replace('.docx', '').replace('.doc', '')}.pdf`;
      ...
      alert("Conversion failed. Please try again or use a correct Word document.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="max-w-7xl mx-auto p-6 relative">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-5xl mx-auto">
        <div className="bg-blue-50/50 border-2 border-dashed border-blue-400 rounded-xl p-8 flex flex-col items-center justify-center text-center min-h-[300px]">
          ...
          <input ref={inputRef} type="file" accept=".doc,.docx,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document" className="hidden" onChange={handleFileChange} />
        </div>
        ...
        <div className="bg-white border rounded-xl shadow p-8 flex flex-col justify-start">
          ...
          {!file ? (
            ...
            <div className="flex items-center justify-between bg-slate-50 border border-slate-200 p-4 rounded-lg shadow-sm hover:border-blue-400 transition group mb-6">
          ...
          <div className="mt-auto space-y-4 border-t pt-4">
            <button onClick={handleConvert} disabled={loading || !file} className="w-full bg-blue-700 text-white py-4 rounded-xl font-bold hover:bg-blue-800 disabled:opacity-50 transition shadow-lg flex justify-center items-center gap-2 text-lg">
